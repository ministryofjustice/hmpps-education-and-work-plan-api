...
class SkillsAndInterestsResourceMapper(
  ...
  fun toCreatePersonalSkillsAndInterestsDto(
    ...
    prisonId: String,
  ): CreatePersonalSkillsAndInterestsDto = with(request) {
  ...
  fun toUpdatePersonalSkillsAndInterestsDto(
    ...
    prisonId: String,
  ): UpdatePersonalSkillsAndInterestsDto = with(request) {
  ...
  private fun toPersonalInterests(personalInterest: PersonalInterest): PersonalInterestDomain = with(personalInterest) {
  ...
  private fun toPersonalInterests(personalInterest: PersonalInterestDomain): PersonalInterest = with(personalInterest) {
    PersonalInterest(
  ...
  private fun toPersonalSkills(personalSkill: PersonalSkill): PersonalSkillDomain = with(personalSkill) {
    PersonalSkillDomain(skillType = toSkillTypeDomain(skillType), skillTypeOther = skillTypeOther)
  }

  private fun toPersonalSkills(personalSkill: PersonalSkillDomain): PersonalSkill = with(personalSkill) {
    PersonalSkill(skillType = toPersonalSkillType(skillType), skillTypeOther = skillTypeOther)
  }

  private fun toInterestTypeDomain(interestType: PersonalInterestType): InterestTypeDomain = when (interestType) {
  ...
  private fun toPersonalInterestType(interestTypeDomain: InterestTypeDomain): PersonalInterestType = when (interestTypeDomain) {
  ...
  private fun toSkillTypeDomain(skillType: PersonalSkillType): SkillTypeDomain = when (skillType) {
  ...
  private fun toPersonalSkillType(skillTypeDomain: SkillTypeDomain): PersonalSkillType = when (skillTypeDomain) {
