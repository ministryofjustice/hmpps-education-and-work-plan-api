...
class WorkOnReleaseEntityMapper {
  ...
  fun fromCreateDtoToEntity(dto: CreateWorkOnReleaseDto): WorkOnReleaseEntity = with(dto) {
    WorkOnReleaseEntity(
  ...
  fun fromEntityToDomain(entity: WorkOnReleaseEntity): WorkOnRelease = with(entity) {
    WorkOnRelease(
      reference = reference,
      hopingToWork = toHopingToWork(hopingToWork),
  ...
  fun updateExistingEntityFromDto(entity: WorkOnReleaseEntity, dto: UpdateWorkOnReleaseDto) = with(entity) {
  ...
  fun toAffectAbilityToWork(affectAbilityToWork: AffectAbilityToWorkEntity): AffectAbilityToWorkDomain = when (affectAbilityToWork) {
  ...
  fun toAffectAbilityToWork(affectAbilityToWork: AffectAbilityToWorkDomain): AffectAbilityToWorkEntity = when (affectAbilityToWork) {
  ...
  fun toHopingToWork(hopingToWork: HopingToWorkDomain): HopingToWorkEntity = when (hopingToWork) {
  ...
  fun toHopingToWork(hopingToWork: HopingToWorkEntity): HopingToWorkDomain = when (hopingToWork) {
