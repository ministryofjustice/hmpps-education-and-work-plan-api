...
class WorkInterestsResourceMapper(
  ...
  private fun toFutureWorkInterests(workInterest: WorkInterest): FutureWorkInterest =
    with(workInterest) {
      FutureWorkInterest(workTypeOther = workTypeOther, role = role, workType = toWorkType(workType))
    }

  private fun toWorkInterests(futureWorkInterest: FutureWorkInterest): WorkInterest =
    with(futureWorkInterest) {
  ...
  private fun toWorkType(workInterestType: WorkInterestType): WorkType =
    when (workInterestType) {
