...
class UpdateActionPlanReviewStatusTest : IntegrationTestBase() {
  ...
  @Test
  fun `should not add 5 days to latest review date when exemption removed given the review was already overdue when the exemption was applied`() {
    // Given
    createReviewScheduleRecord(
      prisonNumber,
      status = ReviewScheduleStatusEntity.EXEMPT_PRISONER_FAILED_TO_ENGAGE,
  ...
  @Test
  fun `should not add 10 days to latest review date when exclusion removed given the review was already overdue when the exclusion was applied`() {
    // Given
    createReviewScheduleRecord(
      prisonNumber,
      status = ReviewScheduleStatusEntity.EXEMPT_PRISON_REGIME_CIRCUMSTANCES,
